...
public abstract class AbstractBlockChain {
    ...
    protected final int NDIFF_DIGISHIELD_ENTRY_BLOCK_TESTNET = 10;
...
private void verifyDifficulty(BigInteger calcDiff, Block nextBlock) throws VerificationException
{
...
private void checkDifficultyTransitions_V2(StoredBlock pindexLast, Block pblock) throws VerificationException,BlockStoreException
{
    ...
    private void checkDifficultyTransitions_V1(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        int accuracyBytes = (int) (nextBlock.getDifficultyTarget() >>> 24) - 3;
        receivedDifficulty = nextBlock.getDifficultyTargetAsInteger();
   ...
   private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
    ...
    if (params.getId().equals(NetworkParameters.ID_TESTNET)) {
        if (storedPrev.getHeight()+1 >= NDIFF_KGW_ENTRY_BLOCK_TESTNET+10) { DiffMode = 2; }
        if (storedPrev.getHeight()+1 >= NDIFF_DIGISHIELD_ENTRY_BLOCK_TESTNET) { DiffMode = 3; }
    }
    else {
    ...
    if		(DiffMode == 1) {
        ...
        checkDifficultyTransitions_V3(storedPrev, nextBlock);
        return;
    }
