...
public class Peer {
    ...
    private void processMessage(MessageEvent e, Message m) throws Exception {
        ...
        if (m instanceof NotFoundMessage) {
            ...
            if (1==2 && vPeerVersionMessage.bestHeight < MIN_PEER_BLOCK_HEIGHT)
            {
                log.warn("Connected to a peer with just {} blocks. Don't accept it.",
                        vPeerVersionMessage.bestHeight);
                e.getChannel().close();
            }
            if (1==2 && !vPeerVersionMessage.subVer.contains(ACCEPTED_SUBVERSION))
            {
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) throws IOException {
        ...
        //
        //   https://en.leafcoin.it/wiki/Protocol_specification#getblocks
        //
